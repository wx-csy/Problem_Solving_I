...
\begin{document}
  ...
  \section{[UD] Problem 3.9}
    ...
    Proof: The contrapositive of the statement is: if $n$ is even, then $3n$ is even. \par
    If $n$ is even, then there exists an integer $k$ s.t. $n=2k$. Therefore, $3n=3(2k)=2(3k)$ where $3k$ is an integer, so $3n$ is even. $\square$

    \section{[UD] Problem 3.11}
    Proof: Suppose $\sqrt{2x}=k$ is an integer, then $2x=k^2$ is  even. \par
    ...
    We have proved that $k$ is even, i.e. there exists an integer $n$ s.t. $k=2n$. So $k^2=4n^2=2x$, i.e. $x=2n^2$ is even, but by assuption $x$ is odd, which leads to a contradiction. So $\sqrt{2x}$ is not an integer.  $\square$
