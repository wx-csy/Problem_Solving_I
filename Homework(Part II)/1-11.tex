...
\begin{document}
  ...
  Suppose that the maximum distance between any two points on a polygon occurs between $M$ and $N$. First, regard $N$ as an arbitrary fixed point, and consider point $M$. \par
  ...
  Now, we have proved that for arbitrary $N$, the length of $NM$ is maximum when $M$ is a vertex of the polygon. Consider point $N$, we can prove that the length of $MN$ is maximum when $N$ is a vertex  of the polygon likewise (Figure 2(c)). Hence, the maximum distance between any two points on a polygon occurs between two of the vertices. \hfill $\square$
 ...
 The output of the program is a number, the maximum distance of two points on the polygon.
  \par
